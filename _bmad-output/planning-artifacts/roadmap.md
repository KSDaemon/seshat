...
> Last updated 2026-06-12. Sources: `epics.md`, `.ralph/tasks/*.md`, codebase analysis.

## Status as of 2026-06-12

All 14 epics (1–12 including 3.5 and 6.5, plus Epic 14) — **COMPLETED**. Fully functional product: scanning, convention detection, MCP server with 9 tools, TUI review wizard, file watcher, branch-aware knowledge graph, auto-scan, init/update/uninstall, project-wide merge-aware decisions with git-state freshness checks. Cross-platform self-update (macOS / Linux / Windows MSVC) shipping via direct curl + Homebrew tap. **Current released version: v0.5.1** (2026-05-27).

**Latest delivery — MCP response quality + scanner robustness (`main`, v0.5.0 → v0.5.1, 2026-05-26..27):** Two dev-specs landed (`dev-spec-phase1-mcp-response-cleanup.md`, `dev-spec-phase2-real-snippets-pipeline.md`). **v0.5.0** — trimmed MCP payload noise (hide `repo`/`scope`/redundant `file_path` from tool schemas; trim convention/dependency/golden output; cap module purpose in `query_project_context`), relevance-gated `validate_approach` (gate rule relevance + restrict duplicate detection to identifier-like tokens to stop false `rules_violated`), `map_diff_impact` base resolved via gitish rev-spec, `query_code_pattern` re-export shadow collapse, bare `crate`/`self`/`super` import resolution. **v0.5.1** — scanner auto-recovers from a stale IR cache instead of hard-failing (NULL-safe stale-IR predicate), stopped leaking `pub mod` / grouped-`use` as external imports, extended `STOP_WORDS` with code-prose filler to kill phantom rules, hardened review patches, Windows path-separator test fix. Plus an ADR post-mortem on the rejected IDF relevance gate (`_bmad-output/implementation-artifacts/validate-approach-relevance-gate-2026-05-27.md`) — IDF is decorative on short-text rule corpora, so hardcoded stop-words shipped instead.

> ⚠️ **CHANGELOG gap:** the `v0.5.0` and `v0.5.1` tags exist in git but `CHANGELOG.md` has no matching `[0.5.0]` / `[0.5.1]` sections — that content still sits under `[Unreleased]`. Reconcile the CHANGELOG before the next release.

**Previous delivery — Manifest parsing batch (branch `feat/manifest-pnpm-fw4`, 2026-05-25):** Two previously-partial manifest items closed. **pnpm-workspace.yaml** (`#jsts-pnpm`) is now wired into `analyze_manifests` — pnpm monorepo members resolve as internal. **FW-4 Poetry/PDM** (`#fw4-alt-backends`) — `parse_pyproject_toml` now reads `[tool.poetry.*]` and `[tool.pdm.dev-dependencies]` so legacy Poetry/PDM declared deps are cross-referenced. Both confined to `crates/seshat-scanner/src/manifest.rs` (non-breaking) → shipped as **v0.4.0** (the `feat:` commits triggered a minor bump under release-plz, not the patch this note originally predicted). FW-5 already shipped in v0.3.2; its stale `[Unreleased]` CHANGELOG block was relocated to a `[0.3.2]` section as part of this work. See `quick-spec-manifest-pnpm-fw4.md`.
